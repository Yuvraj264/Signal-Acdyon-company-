 ...
 * Demonstrates the automated root-cause investigation sequence.
 */
export default function SignalDemo({ onTriggerInvestigation, id = 'demo-workspace' }) {
  ...
  const startInvestigation = () => {
    if (state !== 'idle') return // Guard against rapid clicks
  ...
  return (
    <div id={id} className="relative w-full max-w-xl mx-auto rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-surface)] shadow-sm overflow-hidden">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-[var(--border-subtle)] bg-[var(--bg-subtle)]/70 text-xs">
        <div className="flex items-center gap-2">
          <span className="flex h-2 w-2 rounded-full bg-[var(--accent-signal)] animate-pulse" />
        ...
        <div className="hidden sm:flex items-center gap-1.5 font-mono text-[11px] text-[var(--text-muted)]">
          <Activity className="w-3 h-3 text-[var(--accent-signal)]" />
      ...
      <div className="p-4 sm:p-6 space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 border-b border-[var(--border-subtle)]">
          <div>
            ...
            <div className="text-sm font-semibold text-[var(--text-primary)] flex items-center gap-2 mt-0.5">
              {state === 'idle' && (
                <>
              ...
              {state === 'investigating' && (
                <>
                  <Loader2 className="w-3.5 h-3.5 animate-spin text-[var(--accent-signal)]" />
          ...
          >
            <div className="space-y-4">
              <div className="flex items-center justify-between pb-3 border-b border-[var(--border-subtle)]">
                <div className="flex items-center gap-2">
                  <Terminal className="w-4 h-4 text-[var(--accent-signal)]" />
                ...
                <button
                  ...
                  className="p-1 rounded text-[var(--text-muted)] hover:text-[var(--text-primary)] hover:bg-[var(--bg-subtle)] cursor-pointer"
